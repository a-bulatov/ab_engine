...
При необходимости задать путь поиска с собственным драйвером, можно в строке соединения, в 
фигурных скобках задать параметр **driver_path**, определяющий путь для поиска драйвера.

Если нужно задать свой путь поиска для всех драйверов БД, можно задать параметр **db_driver_path** в секции конфигурации **defaults**.
...
Чтобы задать несколько соединений, нужно использовать database как секцию:
...
При этом, в секции **defaults** можно задать имя соединения БД, которое будет использоваться по умолчанию. Для этого следует задать значение **database** в секции **defaults**.
...
Для некоторых драйверов, например, для встроенного драйвера posstgresql строка сооединения с БД 
может иметь альтернативную структуру:

<имя_драйвера>**://**<имя пользователя>**:**<пароль>**@**<адрес сервера>**:**<номер порта>**/**<имя БД>**?**<параметры соединения>**{**дополнительные параметры**}**

Если нужно установить **peer** соединение c БД postgresql, то в строке соединения достаточно передать имя БД и доп. параметры (если нужно). Например:
```python
...
class Driver:
    ...
    def __init__(self, connection_string, on_open_close=None, notify=None, out_params:Optional[dict]=None):
        ...
        notify список, в который будут записаны уведомления о событиях, либо метод, который будет 
            вызван для каждого события. метод может быть как синхронным, так и асинхронным, на его входе строка.
        out_params словарь выходных параметров. используется в конструкторе драйверов для получения
            разобранного connection_string
        """
        ...
...
Для того чтобы установить PEER соединение с postgresql от имени текущего пользователя, достаточно указать имя драйвера
и имя базы данных:
```yaml config.yaml
...
* NOTICE - то же, что DB, но запросы с такой опцией возвращают как результат список сообщений нотификации
...
Методы RPC регистрируются функцией register_rpc_list либо c помощью декоратора register_rpc, который также можно использовать как функцию.

Регистрация списка функций осуществляется с помощью register_rpc_list, которая принимает на вход параметры defs и self.
* **defs** - содержит список или словарь с описаниями
* **self** - необязательный параметр, в котором передается модуль или объект из которого будут регистрироваться функции. если в self ничего не передано, то будут регистрироваться функции из модуля, в котором вызвана register_rpc_list.

список или словарь с орписаниями содержит элементы следующей структуры:
* **name**: имя функции в API (не нужно и игнорируется, если описание передается в словаре)
...
* **type**: тип функции
  * **self** - функция из объекта, переданного в параметре self
...
Пример списка для регистрации:
```python
from ab_engine import register_rpc_list, call_rpc
...
register_rpc_list({
    ...
    "set":{
        "sql":"set $key $value",
        "ext":"DB(valkey)",
        "help": "устанавливает значение ключа key равным value"
    },
    "get":{
        "sql":"get $key",
        "ext":"DB(valkey)",
        "help": "возвращает значение ключа key"
    }
})

async def test():
    await call_rpc("set", key="test", value=1234)
```
