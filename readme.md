from ab_engine.env import DB_ENVБиблиотека стандартных модулей
==========================================================
...
сохраним этот yaml как my_errors.yaml и в файле рядом напишем код:

```python
from ab_engine import raise_error, load_errors

load_errors("./my_errors.yaml")
...
Из коробки реализован драйвер Postgresql, но позволяет описывать драйвера к другим БД.
...
* **дополнительные параметры**  - параметры, которые будут переданы в сессию соединения с БД. передаются как **"**ключ**"="**значение**"**
    ...
    не станет выполнять очередную команду до тех пор, пока не закончится выполнение любой из выполняемых сейчас команд.
...
class Driver:
    ...
    def __init__(self, connection_string, on_open_close=None):
        ...
        дополнительные параметры строки соединения (те что в фигурных скобках) не передаются в connection_string
        on_open_close функция, передаваемая драйверу, описываемая как async def on_open_close(self, close=False)->dict:
        данная функция возвращает драйверу дополнительные параметры, которые могут быть использованы при создании
        и закрытии соединения (если драйвер это поддерживает) 
        """
        ...
    ...
    async def sql(self, query, one_row=False, row_factory=RowFactory.DICT):
        ...
        row_factory - задает фабрику для получения данных драйвером (задается в ab_engine/db/driver.py RowFactory)
            RowFactory допускает следующие значения:
        ...
        Для выборки возвращает результат с применением соответствующего RowFactory, либо [], если one_row=False и выборка пуста
        Если one_row=True, то вернет только первую строку результата, либо Null, если соответствующая запись не найдена.
        В случае, если query яляется запросом, не возвращающим выборку, вернет целое число - количество записей, 
        на которые повлиял запрос.
        """
...
DB, CALLBACK, TIMEOUT и PAGE это опции, в которых нужно вызывать конструктор. Другие опции можно указывать без вызова конструктора (Без круглых склообок после имени опции).
...
async def test():
    env = DB_ENV()
    x = await env.sql("select $a * $b", a=2, b=5, ONE)
