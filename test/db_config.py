...
async def peer():
    # для peer просто указываем имя БД и, если нужно, дополнительные параметры соединения
    x = await sql("select version()", ONE, DB("postgresql://postgres?application_name=test"))
